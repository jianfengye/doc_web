...
unix实现nagle算法大概有0.2s。

nginx中有个tcp_nodelay配置，就是对Nagle算法进行配置的。如果设置了tcp_nodelay on。那么就代表着，我希望在tcp传输层不要使用nagle算法，发送方不需要等待和储存，有要发送的数据包，直接能进行发送。
...
Crok算法会让网络中传输的包变大，而Nagle算法会让网络中传输的包打包一起传输。
